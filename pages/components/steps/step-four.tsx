...
function StepFour() {
  const [tokens, setTokens] = useState([
    ...
    const [strategy, setStratydy] = useState([
        {"name":"Low Risk","address":"0x","image":[wbtc,weth],"key":"low"},
        {"name":"Middle Risk","address":"0x","image":[lido,op,arb],"key":"middle"},
        {"name":"Middle Risk","address":"0x","image":[wld,snx,link],"key":"heig"},
    ])
      ...
      <div>
        <div className="mx-auto bg-steps px-20 py-10 rounded-2xl">
          <div className="">
            ...
            <p className="font-extrabold text-white mb-10 mt-10 text-5xl">Choose your DCA bundle strategy</p>
            <div className='flex'>
                {strategy?.map(post => <button className='bg-slate-200 gap-4 mx-auto border-green-400 transition w-1/4   h-36 bg flex flex-col items-center justify-center rounded-2xl hover:border-8'>
                    <p className='flex'>{post.image?.map(img => <Image src={img} alt='' width={40} height={40} />)}</p>
                    <p className='text-3xl'>{post.name}</p></button>)}
            </div>
            <p className="font-extrabold text-white mb-10 mt-10 text-5xl">CHECK AND APPROVE INVESTMENT</p>
            <div className='flex'>
            ...
            <div className='text-white flex items-cetner justify-between'>
                <div className=''>
                    ...
                    <input className='bg-gray-200 text-black px-5 rounded-2xl py-2'  type="text" placeholder='$5000'/>
                </div>
                <div className=''>
